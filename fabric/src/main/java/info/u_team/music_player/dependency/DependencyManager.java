...
public class DependencyManager {
	...
	private static final PathLocator PATH_LOCATOR = loadPathLocator();
	
	public static final DependencyClassLoader MUSICPLAYER_CLASSLOADER = new DependencyClassLoader();
	
	/**
	 * Loads the PathLocator service using standard Java ServiceLoader,
	 * replacing the old uteamcore ServiceUtil.loadOne() call.
	 */
	private static PathLocator loadPathLocator() {
		final ServiceLoader<PathLocator> loader = ServiceLoader.load(PathLocator.class);
		return loader.findFirst()
				.orElseThrow(() -> new RuntimeException("Could not find PathLocator service implementation for Music Player"));
	}
